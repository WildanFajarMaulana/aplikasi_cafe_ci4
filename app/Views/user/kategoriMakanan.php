       ...
       <div class="box-menu ">
           ...
           <?php if(!$profilByIdLogin){ ?>
           <a href="/app/profile.html" class="tambah-menu f-pps-l"
               style="text-decoration: none; color: black;">Tambah</a>
           <?php }else{ ?>
           <input type="hidden" class="csrfCafe" name="<?= csrf_token() ?>" value="<?= csrf_hash() ?>" />
           <div class="tambah-menu m-makanan f-pps-l " data-id="<?= $td['id'] ?>">Tambah</div>
           <?php } ?>
       </div>
       <?php } ?>
       <?php }else{ ?>
       <p class="text-center">Menu Sedang Kosong</p>
       <?php } ?>
    ...
    $('.m-makanan').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('.thumbnail-pop-up ').attr("style", "background-image:url(/img/" +
                    response.data[0]['gambar'] + ")");
